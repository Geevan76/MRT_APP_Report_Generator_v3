...
var columnMapping = {
  ...
  '{{Image VInspectionID}}': 25,   // Column Y (index 25)
  ...
  '{{Subsubcomponent}}': 19,       // Column S (index 19)
  '{{Condition}}': 20,             // Column T (index 20)
  '{{Defect Type}}': 21,           // Column U (index 21)
  '{{Remarks}}': 22,               // Column V (index 22)
  '{{Image URL}}': 28,             // Column AB (index 28)
  '{{ImgDescription}}': 27,        // Column AA (index 27)
...
function generateConsolidatedReport(inspections, inspectionOrder, reportPrefix, startItemNo) {
  ...
  // Prepare data rows, respecting the order in the sheet
  for (var i = 0; i < inspectionOrder.length; i++) {
    ...
    // If there are image rows, we add them and ignore the inspection-only row
    if (imageRows.length > 0) {
      for (var j = 0; j < imageRows.length; j++) {
        var imageRow = imageRows[j];
        if (inspectionRow) {
          // Merge inspection data into each image row
    ...
    // If there are no image rows, add the inspection row as is
...
function appendTableRowToDocument(body, row, itemNo) {
  ...
  newRow.getCell(8).setText((row[columnMapping['{{Subsubcomponent}}'] - 1] || "").toString());    // Subcomponent
  newRow.getCell(9).setText((row[columnMapping['{{Condition}}'] - 1] || "").toString());       // Condition
  newRow.getCell(10).setText((row[columnMapping['{{Defect Type}}'] - 1] || "").toString());     // Defect Type
  newRow.getCell(11).setText((row[columnMapping['{{Remarks}}'] - 1] || "").toString());        // Remarks
  ...
  newRow.getCell(12).setText(imgDescription);  // Image Description
