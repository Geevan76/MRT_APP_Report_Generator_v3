...
function generateConsolidatedReport(groupedData, reportPrefix, startItemNo) {
  ...
  var fileName = reportPrefix + "-Inspection_Report_for_" + trainNo + "_" + startItemNo;
  ...
  Logger.log("Batch processed. Start Item No: " + (endItemNo - rowsToAdd.length + 1) + ", End Item No: " + endItemNo);

  return endItemNo;
}

/**
 * Merges inspection data into the image row.
 ...
 * @param {Number} startItemNo - The item number.
 */
function appendTableRowToDocument(body, row, startItemNo) {
  ...
  // Populate cells
  newRow.getCell(0).setText(startItemNo.toString());  // Item No
 ...
 * @param {Object} doc - The Google Doc object.
 * @param {String} sheetName - Name of the sheet ("Visual_Cleaned_Report" or "Functional_Cleaned_Report").
